...
description: Use when financial-services work requires analyze the bond futures basis
  by pricing futures, identifying the cheapest-to-deliver, and comparing with yield
  curves to assess delivery option value and basis trading opportunities. Use when
  analyzing bond futures, computing the basis, identifying CTD bonds, calculating
  implied repo rates, or evaluating basis trades..
version: 0.1.0
author: Changhochien
license: MIT
platforms:
...
This skill was packaged from `pi-financial-services` source path `plugins/partner-built/lseg/skills/bond-futures-basis` for the Hermes financial-services profile. Use institutional data connectors first when available, cite sources, and stage outputs for qualified human review.
